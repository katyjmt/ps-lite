...
function addAnnualPages(nonWeeklyData, selectedPages, pdfFilePaths) {
  ...
  for (let i = 0; i < pageObjects.length; i++) {
    for (let j = 0; j < pageObjects[i].pdf.length; j++) {
      pdfFilePaths.push(pageObjects[i].pdf[j])
    }
  }
...
function addWeeklyAndDailyPages(nonWeeklyData, weeklyData, linedPageData, selectedPages, pdfFilePaths) {
  ...
  for (let i = 0; i < weeklyPageObjects.length; i++) {
    ...
    for (let j = 0; j < weeklyPageObjects[i].pdf.length; j++) {
      pdfFilePaths.push(weeklyPageObjects[i].pdf[j])
      ...
      for (let k = 0; k < matchIso.length; k++) {
        if (k === 0 || k % 2 === 0) {
          pdfFilePaths.push(matchIso[k].pdf[0])
        } else {
          pdfFilePaths.push(matchIso[k].pdf[1])
        }
      }
      if (matchIso % 2 > 0) {
...
// TO DO: UPDATE SO ALL PDFS ARE UNIQUE AND ADDED TO DB UNDER DESIGNS
export async function generatePDFDoc(pdfFilePaths) {
